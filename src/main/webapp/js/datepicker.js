/*Modified by Quake for WebWork UI component*/
/*----------------------------------------------------------------------------\
|                              Date Picker 1.06                               |
|-----------------------------------------------------------------------------|
...
DatePicker.prototype.setDate = function ( oDate, set ) {
	...
	if ( oDate == null ) {
		...
		if ( typeof this.onchange == "function" && set )
			this.onchange();
		return;
	}
...
//added by Quake;
function showDatePicker(field, e){
    //parse field value to date
    var d = new Date();
    var dp = new DatePicker();
    var dtArray = field.value.split('-')
    if(dtArray.length == 3) {
        d = new Date(parseInt(dtArray[0]), parseInt(dtArray[1].substring(0,1)) * 10 + parseInt(dtArray[1].substring(1,2)) - 1, parseInt(dtArray[2].substring(0,1)) * 10 + parseInt(dtArray[2].substring(1,2)), 10, 0, 0, 0)
    }
    
    //clear old div;
    var oldDiv = document.getElementById("_div_dp_" + field.name + "_");
    if(oldDiv != null) document.body.removeChild(oldDiv);

    //create div element
    var ele = document.createElement("div");
    ele.id = "_div_dp_" + field.name + "_";
    ele.style.position = "absolute";

	// left
	var scroll = getScroll();
	if (e.clientX > scroll.width - ele.offsetWidth)
		ele.style.left = scroll.width - ele.offsetWidth + scroll.left + "px";
	else
		ele.style.left = e.clientX - 2 + scroll.left + "px";
	
	// top
	if (e.clientY + ele.offsetHeight + 13 < scroll.height)
		ele.style.top = e.clientY + 13 + scroll.top + "px";
	else if (e.clientY - ele.offsetHeight > 0)
		ele.style.top = e.clientY + scroll.top - ele.offsetHeight + "px";
	else
		ele.style.top = scroll.top + 5 + "px";            
    ...
    //set date and display div;
    dp.setDate(d);
    ele.appendChild(dp.create());
