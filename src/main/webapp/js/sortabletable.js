...
SortableTable.getCellIndex = function (oTh) {
	var cells = oTh.parentNode.childNodes
	var l = cells.length;
	var i;
	for (i = 0; cells[i] != oTh && i < l; i++)
		;
	return i;
};
...
SortableTable.prototype.getValueFromString = function (sText, sType) {
	switch (sType) {
		case "Number":
      var i = sText.lastIndexOf(",");
      if (i >=0 ) {
        var mainSb = sText;
        while (i >= 0) {
            mainSb = sText.substring(0,i) + mainSb.substring(i+1);
            i = sText.lastIndexOf(",", i - 1);
        }
        sText=mainSb;
      }
			return Number(sText);
